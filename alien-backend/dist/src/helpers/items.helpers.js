...
function getAllItems(filePath) {
    return file_reader_helper_1.jsonReader.getWholeJson(filePath).pipe((0, operators_1.catchError)((err) => {
...
function addItem(item, filePath) {
    return file_writer_helper_1.jsonWriter.addObject(filePath, item).pipe((0, operators_1.catchError)((err) => {
        if (err.message === 'Error during file reading.') {
            return file_writer_helper_1.jsonWriter
                .createJSON(filePath, item)
                .pipe((0, operators_1.catchError)((err) => (0, rxjs_1.of)(err)));
        }
        return (0, rxjs_1.of)(err);
    }));
}
...
function editItem(item, id, filePath) {
    return file_writer_helper_1.jsonWriter.editObject(filePath, item, id).pipe((0, operators_1.catchError)((err) => {
...
function deleteItem(id, filePath) {
    return file_writer_helper_1.jsonWriter.deleteObject(filePath, id).pipe((0, operators_1.catchError)((err) => {
        if (err.message === 'Error during renaming.') {
            throw new common_1.HttpException({ successful: false, result: `Item with id - ${id} was not found` }, common_1.HttpStatus.NOT_FOUND);
        }
        return (0, rxjs_1.of)(err);
